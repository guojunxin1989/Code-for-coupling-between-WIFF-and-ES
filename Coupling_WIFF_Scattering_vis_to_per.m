...
v(BN+1,FN)=w/real(ke);
inQ(BN+1,FN)=2*imag(ke)/real(ke);

end

end

load vscafile1 v_sca;
load inQscafile1 inQ_sca;
...
hold on;
semilogx(f,v(5,:),'k','linewidth',1.5);

xlabel('Frequency (Hz)','FontSize',12);
ylabel('{\itP}-wave velocity (m/s)','FontSize',12);
legend('{\itb}{\rightarrow}{\infty}','{\itb}=10^{12} Pa*s/m^{2}','{\itb}=10^{11} Pa*s/m^{2}','{\itb}=10^{10} Pa*s/m^{2}','{\itb}=10^{9} Pa*s/m^{2}');
set(gca, 'FontSize', 12);
xlim([0.01 100000]);
...
hold on;
loglog(f,inQ(5,:),'k','linewidth',1.5);
